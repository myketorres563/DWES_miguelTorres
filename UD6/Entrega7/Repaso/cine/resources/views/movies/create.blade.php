<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Nueva película</h2>
    </x-slot>
            ...
            <div class="bg-white p-6 shadow rounded">
                <form method="POST" action="{{ route('movies.store') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input name="year" value="{{ old('year') }}" class="w-full border rounded p-2">
                        @error('year') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block font-medium">Género</label>
                        <input name="genre" value="{{ old('genre') }}" class="w-full border rounded p-2">
                        @error('genre') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block font-medium">Sinopsis</label>
                        <textarea name="synopsis" class="w-full border rounded p-2" rows="4">{{ old('synopsis') }}</textarea>
                    ...
                    <div class="flex gap-3">
                        ...
                        <a class="px-4 py-2 bg-gray-200 rounded" href="{{ route('movies.index') }}">Cancelar</a>
                    </div>
                </form>
            </div>
        </div>
